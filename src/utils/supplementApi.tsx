...
const extractProductKeywords = (ocrText: string): string[] => {
    const koreanText = ocrText.replace(/\s+/g, '').match(/[가-힣]+/g) || [];
...
export const fetchMatchedSupplementData = async (ocrText: string) => {
    ...
    try {
        // 1. OCR 텍스트에서 공백 제거
        const normalizedOCRText = ocrText.replace(/\s+/g, '').match(/[가-힣]+/g)?.join('') || ''; // 공백 제거
        // 2. 키워드 추출
        ...
        const exactMatch = products.find((product: any) => {
            const normalizedProductName = product.PRDLST_NM.replace(/\s+/g, '') || '';
        ...
        products.forEach((product: any) => {
            ...
            console.log("product:",product.PRDLST_NM);
